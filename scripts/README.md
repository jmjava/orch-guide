...
| `assert-spdd-projection-pin.sh` | Fail-closed check that `refs/tags/spdd-projection-v3` exists and peels to the SHA in `spdd-projection-pin.env`. `--self-test` proves missing-tag, wrong-SHA, `silent-readme-edit-does-not-retag`, and `pin-stays-187d4d4`. Leftover #8: tag bump is a deliberate leftover. Does not retag. |
| `forbid-embabel-upstream.sh` | Fail-closed check that no remote can push to `embabel/guide`. `--fix` keeps fetch-from-Embabel and sets push to `DISABLED`. `--leftover-text` refuses leftover notes that open an Embabel PR, contribute absorption, or clean Layer B for Embabel. `--self-test` proves `default-push-url-fails`, `pre-push-url-fails`, `fix-keeps-fetch`, `fix-disables-push`, `cursor-rule-alwaysApply`, deleting the rule, dropping `alwaysApply`, `absorption-doc-fork-local`, `no-leftover-asks-to-upstream`, `leftover-asks-to-upstream-fails`, `cloud-agent-env-fork-local`, `do-not-pr-embabel`, `leftover-that-cleans-layer-b-for-embabel-is-refused`, `feat-work-as-embabel-mr-is-refused`, `contribute-absorption-is-refused`, and `do-not-open-prs-small-or-large`. `FORBID_GIT_ROOT` points the remote scan at a throwaway repo. `FORBID_CURSOR_RULE` overrides the Cursor rule path. `FORBID_ABSORPTION_DOC` overrides the absorption-doc path. `FORBID_CLOUD_AGENT_ENV` overrides the cloud-agent-env path. `FORBID_LEFTOVER_TEXT` supplies leftover note text. |
