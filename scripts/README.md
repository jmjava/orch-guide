...
| `forbid-embabel-upstream.sh` | Fail-closed check that no remote can push to `embabel/guide`. `--fix` keeps fetch-from-Embabel and sets push to `DISABLED`. `--self-test` proves `default-push-url-fails`, `pre-push-url-fails`, `fix-keeps-fetch`, `fix-disables-push`, `cursor-rule-alwaysApply`, deleting the rule, dropping `alwaysApply`, `absorption-doc-fork-local`, `no-leftover-asks-to-upstream`, and `leftover-asks-to-upstream-fails`. `FORBID_GIT_ROOT` points the remote scan at a throwaway repo. `FORBID_CURSOR_RULE` overrides the Cursor rule path. `FORBID_ABSORPTION_DOC` overrides the absorption-doc path. |
